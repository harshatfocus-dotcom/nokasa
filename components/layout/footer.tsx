...
import { Phone, Leaf, Package, Truck } from "lucide-react";
...
const PLATFORM_LINKS = [
  ...
  { label: "Guide on Selling Clothes", href: "https://nokasa.co/blogs/selling-clothes-in-india/", external: true },
  { label: "Book a Slot", href: "/book-a-slot" },
];
...
const SOCIAL = [
  ...
  {
    label: "Facebook",
    href: "https://www.facebook.com/nokasa",
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
...
function FooterAnimation() {
  ...
  const [stage, setStage] = useState<"idle" | "atPickup" | "driving" | "atPickedUp" | "atSegregated" | "atSecondLife">("idle");
  ...
  const startAnimation = async () => {
    ...
    // 1. Drive to Book a pickup (10%)
    await controls.start({ x: "5vw", transition: { duration: 2.5, ease: [0.16, 1, 0.3, 1] } });
    setStage("atPickup");
    await new Promise(resolve => setTimeout(resolve, 1600));

    // 2. Drive to It gets picked up (33%)
    setStage("driving");
    await controls.start({ x: "28vw", transition: { duration: 3, ease: "linear" } });
    setStage("atPickedUp");
    await new Promise(resolve => setTimeout(resolve, 1600));

    // 3. Drive to Segregated (57%)
    setStage("driving");
    await controls.start({ x: "52vw", transition: { duration: 3, ease: "linear" } });
    setStage("atSegregated");
    await new Promise(resolve => setTimeout(resolve, 1600));

    // 4. Drive to Second life (81%)
    setStage("driving");
    await controls.start({ x: "76vw", transition: { duration: 3, ease: "linear" } });
    setStage("atSecondLife");
    await new Promise(resolve => setTimeout(resolve, 1600));

    // Exit
    setStage("driving");
    await controls.start({ x: "135vw", transition: { duration: 1.5, ease: "easeIn" } });
    setStage("idle");
  ...
  const checkpointBase = "absolute bottom-[28px] flex flex-col items-center z-30";
  const iconBox = "w-8 h-8 rounded-full bg-white dark:bg-gray-900 border border-gray-100 dark:border-white/10 flex items-center justify-center shadow-sm mb-2";

  const passedPickup = stage === "atPickup" || stage === "driving" || stage === "atPickedUp" || stage === "atSegregated" || stage === "atSecondLife";
  const passedPickedUp = stage === "atPickedUp" || stage === "atSegregated" || stage === "atSecondLife";
  const passedSegregated = stage === "atSegregated" || stage === "atSecondLife";

  return (
    <div ref={containerRef} className="relative w-full h-44 md:h-52 bg-transparent flex flex-col justify-end pb-12 overflow-hidden select-none px-4 md:px-0">
      {/* Road line */}
      <div className="absolute bottom-[48px] left-0 right-0 h-[1px] bg-gray-100 dark:bg-white/5" />

      {/* Checkpoint 1: Book a pickup — 10% */}
      <div className={checkpointBase} style={{ left: "10%", transform: "translateX(-50%)" }}>
        <div className={iconBox}>
          <div className="relative">
            ...
            <motion.div
              ...
              initial={{ scale: 0 }}
              animate={passedPickup ? { scale: 1 } : { scale: 0 }}
        ...
        <span className="text-[10px] font-black tracking-[0.15em] text-gray-400 dark:text-gray-500 uppercase text-center leading-tight">Book a<br />pickup</span>
      </div>

      {/* Checkpoint 2: It gets picked up — 33% */}
      <div className={checkpointBase} style={{ left: "33%", transform: "translateX(-50%)" }}>
        <div className={iconBox}>
          <div className="relative">
            ...
            <motion.div
              className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-emerald-500 flex items-center justify-center"
              initial={{ scale: 0 }}
              animate={passedPickedUp ? { scale: 1 } : { scale: 0 }}
              transition={{ type: "spring", stiffness: 400, damping: 15 }}
            >
              <svg width="6" height="6" viewBox="0 0 8 8" fill="none">
                <path d="M1.5 4L3 5.5L6.5 2" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        ...
        <span className="text-[10px] font-black tracking-[0.15em] text-gray-400 dark:text-gray-500 uppercase text-center leading-tight">It gets<br />picked up</span>
      </div>

      {/* Checkpoint 3: Segregated — 57% */}
      <div className={checkpointBase} style={{ left: "57%", transform: "translateX(-50%)" }}>
        <div className={iconBox}>
          ...
          <>
            {["🧥", "👕", "👗"].map((_item, i) => (
              <motion.span
                ...
                style={{ bottom: "48px", left: `calc(50% + ${(i - 1) * 14}px)` }}
                initial={{ opacity: 0, y: 0, scale: 0.5 }}
                animate={stage === "atSegregated" ? { opacity: [0, 1, 1, 0], y: [-4, -18, -24, -28], scale: 1 } : { opacity: 0, y: 0, scale: 0.5 }}
        ...
        <motion.div
          className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-emerald-500 flex items-center justify-center"
          initial={{ scale: 0 }}
          animate={passedSegregated ? { scale: 1 } : { scale: 0 }}
          transition={{ type: "spring", stiffness: 400, damping: 15 }}
          style={{ top: "auto", right: "auto", position: "relative", width: 0, height: 0 }}
        />
        <span className="text-[10px] font-black tracking-[0.15em] text-gray-400 dark:text-gray-500 uppercase text-center leading-tight">Segregated</span>
      </div>

      {/* Checkpoint 4: Second life — 81% */}
      <div className={checkpointBase} style={{ left: "81%", transform: "translateX(-50%)" }}>
        <div className={iconBox}>
          <Leaf className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
        </div>
        {mounted && (
          <motion.div
            className="absolute bottom-[48px] text-emerald-500"
            initial={{ scale: 0, opacity: 0 }}
            animate={stage === "atSecondLife" ? { scale: [0, 1.3, 1], opacity: [0, 1, 0.8] } : { scale: 0, opacity: 0 }}
            transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
          >
            <Leaf size={20} />
          </motion.div>
        )}
        <span className="text-[10px] font-black tracking-[0.15em] text-gray-400 dark:text-gray-500 uppercase text-center leading-tight">Second<br />life</span>
      </div>

      {/* Bloom trail while driving */}
      {mounted && Array.from({ length: 10 }).map((_, i) => {
        const x = 10 + i * 8;
        return (
          <motion.div
            key={i}
            className="absolute bottom-12 text-emerald-500/60 dark:text-emerald-400/30 pointer-events-none"
            style={{ left: `${x}%` }}
            initial={{ opacity: 0, scale: 0 }}
            animate={stage === "driving" || stage === "atPickedUp" || stage === "atSegregated" || stage === "atSecondLife" ? { opacity: 0.35, scale: 1 } : { opacity: 0, scale: 0 }}
      ...
      {/* Truck */}
      <motion.div className="absolute bottom-12 z-20" animate={controls}>
        <motion.div
          className="relative"
          animate={{ y: [0, -0.8, 0] }}
    ...
    <footer className="bg-background border-t border-gray-100 dark:border-white/5 pb-8 transition-colors duration-500">
      ...
      <Container>
        <div className="pt-12 pb-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1.5fr_1fr_1fr_1fr] gap-8 md:gap-10 lg:gap-12 xl:gap-16">
            {/* Branding */}
            <div className="space-y-6">
              ...
              <p className="text-[14px] leading-relaxed text-gray-400 font-medium">
                NoKasa gives your forgotten clothes a second chance and a new purpose. No waiting, no waste, just a simple way to declutter, do good, and get paid.
              </p>
              {/* Contact info */}
              <div className="space-y-2">
                <a href="mailto:support@nokasa.co" className="flex items-center gap-2 text-[13px] text-gray-400 hover:text-brand transition-colors font-medium">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/>
                  </svg>
                  support@nokasa.co
                </a>
                <a href="tel:+916363005509" className="flex items-center gap-2 text-[13px] text-gray-400 hover:text-brand transition-colors font-medium">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.56 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.35a16 16 0 0 0 6.29 6.29l1.42-1.42a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/>
                  </svg>
                  +91 6363 005 509
                </a>
              </div>
              ...
              <ul className="space-y-4">
                {PLATFORM_LINKS.map(link => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="text-[14px] font-bold text-gray-500 dark:text-gray-300 hover:text-brand transition-colors"
            ...
            <div className="flex flex-col gap-6">
              <div>
                <h4 className="text-[15px] font-black text-gray-950 dark:text-gray-100 mb-2 tracking-tight">Eco-logistics.</h4>
              ...
              <Link
                href="/book-a-slot"
                className="py-4 px-6 bg-gray-950 dark:bg-brand text-white rounded-2xl text-[12px] font-black uppercase tracking-widest text-center hover:bg-brand transition-all duration-300 shadow-lg shadow-gray-200 dark:shadow-none w-full"
              >
                Start Pickup
              </Link>
            </div>
        ...
        <div className="py-6 border-t border-gray-100 dark:border-white/5 flex flex-col md:flex-row items-center justify-between gap-6">
          <p className="text-[12px] text-gray-300 font-medium">© {new Date().getFullYear()} NoKasa Technologies Pvt. Ltd. All Rights Reserved.</p>
        </div>
      </Container>
    </footer>
  );
